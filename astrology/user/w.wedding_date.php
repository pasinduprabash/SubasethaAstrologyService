...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $service_name = "Wedding Date Selection";
    $price = "2000";

    $sql = "INSERT INTO wedding_date(user_id, boy_full_name, boy_phone, boy_dob, boy_pob, boy_tob, boy_nakatha, girl_full_name, girl_phone, girl_dob, girl_pob, girl_tob, girl_nakatha, year, month, service_name, price) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = $conn->prepare($sql);

    if ($stmt) {
        $stmt->bind_param("sssssssssssssssss", $loggedInUser, $boy_full_name, $boy_phone, $boy_dob, $boy_pob, $boy_tob, $boy_nakatha, $girl_full_name, $girl_phone, $girl_dob, $girl_pob, $girl_tob, $girl_nakatha, $year, $month, $service_name, $price);
    ...
    <div class="container">
        <div class="left-section">
            ...
            <div class="form-group row">
                ...
                <h5 class="form-title">Wedding Date</h5>

                <div class="form-group row">
                    <div class="col-md-6">
                        <label for="year">Expecting Year:</label>
                        <input type="number" id="year" name="year" class="form-control" min="2025" max="2100" required>
                    </div>

                    <div class="col-md-6">
                        <label for="year">Expecting Month:</label>
                        <select id="month" name="month" class="form-control" required>
                            <option value="" disabled selected>Expecting Month</option>
                            <option value="jan-june">January to June</option>
                            <option value="june-december">June to December</option>
                        </select>
                    </div>
                </div> 
